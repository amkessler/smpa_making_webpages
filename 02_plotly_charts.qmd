...
all_cities_data <- txhousing %>%
  group_by(city)

all_cities_plotlychart <- all_cities_data %>%
...
san_antonio_data <- all_cities_data %>%
...
city_layer <- function(plot_obj, city_name) {
  plot_obj %>%
    add_lines(
      data = filter(all_cities_data, city == city_name),
      x = ~date,
      y = ~median,
      name = city_name
    )
